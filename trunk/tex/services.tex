...
Dienste, die im Netzwerk zur Verfügung stehen (Netzwerkdienste), wie ein Web- oder \glossary{name={FTP}, description={File Transfer Protocol}}{FTP}-Server , lassen sich einfach / simpel direkt über das Netz auf ihren Zustand (hin) überprüfen /  testen.
...
Falls es sich beim Client um ein Unixderivat handelt, ist der entfernte Zugriff auf diesen Client per SSH\footnote{Durch eine Secure Shell (SSH) \glossary{name={SSH}, description={Secure Shell}} kann man sich eine verschlüsselte Netzwerkverbindung zum entfernten Rechner aufbauen.}-Dienst möglich.
...
Eine alternative Möglichkeit solche Dienste auf entfernten Rechnern zu überwachen, ist durch den sogenannten Nagios Remote Plugin Executor (NRPE) \glossary{name={NRPE}, description={Nagios Remote Plugin Executor}}.
...
Durch SNMP kann auf die strukturierte Datenhaltung der MIB\footnote{Die Management Information Base (MIB) \glossary{name={MIB}, description={Management Information Base}} dient als SNMP-Informationstruktur und besteht aus einem hierarchischen, aus Zahlen aufgebauten Namensraum. Ähnliche Struktur wie andere hierarchische Verzeichnisdiensten wie DNS \glossary{name={DNS}, description={Domain Name System}} oder LDAP \glossary{name={LDAP}, description={Lightweight Directory Access Protocol}}. Quelle: \cite{Barth08} S.233} in den entfernten Netzwerkknoten zugegriffen werden.
...
Auf der anderen Seite muss der \pictext{NSCA} (Nagios Service Check Acceptor) \glossary{name={NSCA}, description={Nagios Service Check Acceptor}} als Dämon gestartet sein, damit die übermittelten Ergebnisse entgegengenommen werden können.
\begin{figure}[ht]
	\centering
